...
func (rk *RemoteKV) Delete() error {
	...
	_, err := rk.kv.Delete(context.Background(), rk.key)
	rk.kv = nil
	return err
}

func (rk *RemoteKV) Put(val string) error {
	_, err := rk.kv.Put(context.Background(), rk.key, val)
	return err
}
...
func NewUniqueKey(kv v3.KV, prefix string) (*RemoteKV, error) {
	return NewUniqueKV(kv, prefix, "", v3.NoLease)
}
...
func putNewKV(kv v3.KV, key, val string, leaseID v3.LeaseID) (int64, error) {
	...
	txnresp, err := kv.Txn(context.Background()).If(cmp).Then(req).Commit()
	if err != nil {
		return 0, err
	}
...
func NewSequentialKV(kv v3.KV, prefix, val string) (*RemoteKV, error) {
	return newSequentialKV(kv, prefix, val, v3.NoLease)
}
...
func newSequentialKV(kv v3.KV, prefix, val string, leaseID v3.LeaseID) (*RemoteKV, error) {
	resp, err := kv.Get(context.Background(), prefix, v3.WithLastKey()...)
	if err != nil {
		return nil, err
	}
	...
	newKey := fmt.Sprintf("%s/%020d", prefix, newSeqNum)
